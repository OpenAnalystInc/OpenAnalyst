...
import { editFileTool } from "../tools/editFileTool" // oacode_change: Morph fast apply
...
import { reportExcessiveRecursion, yieldPromise } from "../oacode"
...
export async function presentAssistantMessage(cline: Task, recursionDepth: number = 0 /*oacode_change*/) {
	reportExcessiveRecursion("presentAssistantMessage", recursionDepth) // oacode_change
			...
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "search_and_replace":
						return `[${block.name} for '${block.params.path}']`
					// oacode_change start: Morph fast apply
					case "edit_file":
