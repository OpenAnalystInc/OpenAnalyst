...
import { getProjectRooDirectoryForCwd } from "../../../services/roo-config" // oacode_change
...
describe("CustomModesManager", () => {
	...
	describe("getCustomModes", () => {
		...
		it("should merge modes with .oacodemodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .oacodemodes since it takes precedence
		...
		it("should handle missing .oacodemodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .oacodemodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .oacodemodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .oacodemodes precedence", async () => {
			...
			// Should update global state with merged modes where .oacodemodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .oacodemodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .oacodemodes file when adding project-specific mode", async () => {
			...
			// Verify .oacodemodes was created with the project mode
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".oacodemodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".oacodemodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(
						getProjectRooDirectoryForCwd(mockWorkspacePath), // oacode_change
					)
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".oacode", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should work with global custom modes when .roomodes doesn't exist", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.oacode/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.oacode/rules-code" },
			])
