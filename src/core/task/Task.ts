...
export type TaskOptions = {
	context: vscode.ExtensionContext // oacode_change
...
type UserContent = Array<Anthropic.ContentBlockParam> // oacode_change

export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	private context: vscode.ExtensionContext // oacode_change
	...
	constructor({
		...
		super()
		this.context = context // oacode_change
		...
		this.taskIsFavorited = historyItem?.isFavorited // oacode_change
		// Normal use-case is usually retry similar history task with new workspace.
		this.workspacePath = parentTask
			? parentTask.workspacePath
			: getWorkspacePath(path.join(os.homedir(), "Documents")) // oacode_change: use Documents instead of Desktop as default
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		// oacode_change start: no cloud service
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[OaCode#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[OpenAnalyst#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async sayAndCreateMissingParamError(toolName: ToolName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`OpenAnalyst tried to use ${toolName}${
	...
	public async recursivelyMakeClineRequests(
		...
		recursionDepth: number = 0, // oacode_change
	): Promise<boolean> {
		reportExcessiveRecursion("recursivelyMakeClineRequests", recursionDepth) // oacode_change
		if (this.abort) {
			throw new Error(`[OaCode#recursivelyMakeClineRequests] task ${this.taskId}.${this.instanceId} aborted`)
		}
		...
		const [parsedUserContent, needsRulesFileCheck] = await processOaUserContentMentions({
		...
		if (needsRulesFileCheck) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.oacode/rules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory",
			)
		}
		// oacode_change end
		...
		try {
			...
			let usageMissing = false // oacode_change
			...
			const updateApiReqMsg = (cancelReason?: ClineApiReqCancelReason, streamingFailedMessage?: string) => {
				// oacode_change start: pending upstream pr https://github.com/RooCodeInc/Roo-Code/pull/6122
			...
			try {
				// oacode change: use manual iterator instead of for ... of
			...
			// oacode_change: pending upstream pr https://github.com/RooCodeInc/Roo-Code/pull/6122
			...
			if (assistantMessage.length > 0) {
				...
				// oacode_change start: prevent excessive recursion
	...
	/*private oacode_change*/ async getSystemPrompt(): Promise<string> {
	...
	public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
		...
		try {
			...
			// oacode_change start
			// Check for payment required error from OaCode provider
			if ((error as any).status === 402 && apiConfiguration?.apiProvider === "oacode") {
				const balance = (error as any).balance ?? "0.00"
				const buyCreditsUrl = (error as any).buyCreditsUrl ?? "https://oacode.ai/profile"

				const { response } = await this.ask(
					"payment_required_prompt",
					JSON.stringify({
						title: t("oacode:lowCreditWarning.title"),
						message: t("oacode:lowCreditWarning.message"),
						balance: balance,
