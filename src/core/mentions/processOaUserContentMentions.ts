...
export async function processOaUserContentMentions({
	...
	// Track if we need to check oarules file
	...
	const processUserContentMentions = async () => {
		...
		const { localWorkflowToggles, globalWorkflowToggles } = await refreshWorkflowToggles(context, cwd) // oacode_change
			...
			userContent.map(async (block) => {
				...
				if (block.type === "text") {
					if (shouldProcessMentions(block.text)) {
						// oacode_change begin: pull slash commands from Cline
