...
//oacode_change start
import { McpDownloadResponse, McpMarketplaceCatalog } from "../../shared/oacode/mcp"
import { McpServer } from "../../shared/mcp"
import { OpenRouterHandler } from "../../api/providers"
import { stringifyError } from "../../shared/oacode/errorUtils"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		// oacode_change start: extract constant inTabMode
		...
		this.getState().then(
			({
				...
				terminalShellIntegrationDisabled = true, // oacode_change: default
	...
	public async initClineWithTask(
		...
		const task = new Task({
			context: this.context, // oacode_change
			provider: this,
			apiConfiguration,
			enableDiff,
			enableCheckpoints,
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		const csp = [
			...
			`img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com data: https://*.googleusercontent.com https://*.googleapis.com https://*.githubusercontent.com`, // oacode_change: add https://*.googleusercontent.com and https://*.googleapis.com and https://*.githubusercontent.com
		...
		*/
        ...
        <html lang="en">
          <head>
			...
			<!-- oacode_change: add https://*.googleusercontent.com https://*.googleapis.com https://*.githubusercontent.com to img-src, https://* to connect-src -->
	...
	async upsertProviderProfile(
		...
		try {
			...
			if (activate) {
				...
				await TelemetryService.instance.updateIdentity(providerSettings.oacodeToken ?? "") // oacode_change
			} else {
				await this.updateGlobalState("listApiConfigMeta", await this.providerSettingsManager.listConfig())
			}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\Oa-Code\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "Oa-Code", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Oa-Code/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Oa-Code", "MCP")
		} else {
			// Linux: ~/.local/share/Oa-Code/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "Oa-Code", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".oacode", "mcp")
		}
	...
	// oacode_change:
	async handleOaCodeCallback(token: string) {
		const oacode: ProviderName = "oacode"
		let { apiConfiguration, currentApiConfigName } = await this.getState()
		...
		vscode.window.showInformationMessage("OpenAnalyst successfully configured!")
	...
	async handleEnhancedOaCodeCallback(callbackData: any) {
		try {
			...
			// Use auth state manager to get clean user display
	...
	async validateOACodeToken(): Promise<{
		...
		try {
			...
			const expiresIn = await authStateManager.getTimeUntilExpiration()
	...
	async checkTokenHealth(warningMinutes: number = 60): Promise<{
		...
		try {
			...
			if (validation.expiresIn !== undefined && validation.expiresIn < warningMinutes) {
				return {
					needsAttention: true,
	...
	async getTaskWithId(id: string): Promise<{
		...
		if (historyItem) {
			...
			if (fileExists) {
				...
				) //oacode_change show extra debugging information to debug task not found issues
			}
		} else {
			vscode.window.showErrorMessage(`Task with ID: ${id} not found in history.`) // oacode_change show extra debugging information to debug task not found issues
		}

		// if we tried to get a task that doesn't exist, remove it from state
		// FIXME: this seems to happen sometimes when the json file doesnt save to disk for some reason
		// await this.deleteTaskFromState(id) // oacode_change disable confusing behaviour
		await this.setTaskFileNotFound(id) // oacode_change
	...
	async getStateToPostToWebview() {
		const {
			...
			terminalZdotdir,
			fuzzyMatchThreshold,
			// mcpEnabled,  // oacode_change: always true
			...
			ghostServiceSettings, // oacode_changes
			...
			dismissedNotificationIds, // oacode_change
		...
		return {
			...
			uiKind: vscode.UIKind[vscode.env.uiKind], // oacode_change
			...
			shouldShowAnnouncement: false, // oacode_change
			...
			terminalShellIntegrationDisabled: terminalShellIntegrationDisabled ?? true, // oacode_change: default
			...
			mcpEnabled: true, // oacode_change: always true
			...
			showAutoApproveMenu: showAutoApproveMenu ?? false, // oacode_change
			showTaskTimeline: showTaskTimeline ?? true, // oacode_change
			language, // oacode_change
			renderContext: this.renderContext,
			maxReadFileLine: maxReadFileLine ?? -1,
			maxImageFileSize: maxImageFileSize ?? 5,
			maxTotalImageSize: maxTotalImageSize ?? 20,
			maxConcurrentFileReads: maxConcurrentFileReads ?? 5,
			allowVeryLargeReads: allowVeryLargeReads ?? false, // oacode_change
			...
			organizationAllowList,
			ghostServiceSettings: ghostServiceSettings ?? {}, // oacode_change
			organizationSettingsVersion,
			condensingApiConfigId,
			customCondensingPrompt,
			...
			systemNotificationsEnabled: systemNotificationsEnabled ?? false, // oacode_change
			dismissedNotificationIds: dismissedNotificationIds ?? [], // oacode_change
	...
	async getState() {
		...
		return {
			...
			alwaysAllowUpdateTodoList: stateValues.alwaysAllowUpdateTodoList ?? true, // oacode_change
			...
			terminalShellIntegrationDisabled: stateValues.terminalShellIntegrationDisabled ?? true, // oacode_change: default
			...
			commitMessageApiConfigId: stateValues.commitMessageApiConfigId, // oacode_change
			terminalCommandApiConfigId: stateValues.terminalCommandApiConfigId, // oacode_change
			ghostServiceSettings: stateValues.ghostServiceSettings ?? {}, // oacode_change
			...
			showAutoApproveMenu: stateValues.showAutoApproveMenu ?? false, // oacode_change
			showTaskTimeline: stateValues.showTaskTimeline ?? true, // oacode_change
			maxReadFileLine: stateValues.maxReadFileLine ?? -1,
			maxImageFileSize: stateValues.maxImageFileSize ?? 5,
			maxTotalImageSize: stateValues.maxTotalImageSize ?? 20,
			maxConcurrentFileReads: stateValues.maxConcurrentFileReads ?? 5,
			allowVeryLargeReads: stateValues.allowVeryLargeReads ?? false, // oacode_change
			systemNotificationsEnabled: stateValues.systemNotificationsEnabled ?? true, // oacode_change
			dismissedNotificationIds: stateValues.dismissedNotificationIds ?? [], // oacode_change
	...
	async resetState() {
		...
		// Logout from OpenAnalyst provider before resetting (same approach as ProfileView logout)
		const { apiConfiguration, currentApiConfigName } = await this.getState()
		if (apiConfiguration.oacodeToken) {
			await this.upsertProviderProfile(currentApiConfigName, {
				...apiConfiguration,
				oacodeToken: "",
			})
	...
	public async getTelemetryProperties(): Promise<TelemetryProperties> {
		...
		return {
			...
			platform: isWsl ? "wsl" /* oacode_change */ : process.platform,
