	...
	switch (message.type) {
		...
		case "fetchProfileDataRequest":
			try {
				...
				const response = await axios.get("https://oacode.ai/api/profile", {
					headers: {
				...
				provider.postMessageToWebview({
					...
					payload: { success: true, data: { oacodeToken, ...response.data } },
				})
			} catch (error: any) {
				const errorMessage =
		...
		case "fetchBalanceDataRequest": // New handler
			try {
				...
				const response = await axios.get("https://oacode.ai/api/profile/balance", {
