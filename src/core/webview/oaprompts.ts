 ...
 */
export async function createPromptFile(filename: string, isGlobal: boolean, category: string): Promise<void> {
	// Input validation
	...
	try {
		...
		if (await fileExistsAtPath(filePath)) {
			throw new Error(
				`Prompt file "${finalFilename}" already exists in ${isGlobal ? "global" : "workspace"} scope`,
			)
		}
	} catch (error) {
		...
		throw new Error(
			`Failed to create prompt directory: ${error instanceof Error ? error.message : "Unknown error"}`,
		)
	}
 ...
 */
export async function deletePromptFile(promptName: string, source: "workspace" | "global"): Promise<void> {
	// Input validation
 ...
 * @param source - Source scope ("workspace" or "global")
 */
export async function editPromptBlock(promptName: string, source: "workspace" | "global"): Promise<void> {
	// Input validation
	if (!promptName || typeof promptName !== "string" || promptName.trim().length === 0) {
		throw new Error("Prompt name is required and must be a non-empty string")
	}
	...
	try {
		...
		throw new Error(
			`Failed to open prompt file for editing: ${error instanceof Error ? error.message : "Unknown error"}`,
		)
	}
}
