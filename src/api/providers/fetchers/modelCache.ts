...
export /*oacode_change*/ async function writeModels(router: RouterName, data: ModelRecord) {
	const filename = `${router}_models.json`
	const cacheDir = await getCacheDirectoryPath(ContextProxy.instance.globalStorageUri.fsPath)
	await safeWriteJson(path.join(cacheDir, filename), data)
}

export /*oacode_change*/ async function readModels(router: RouterName): Promise<ModelRecord | undefined> {
	const filename = `${router}_models.json`
	const cacheDir = await getCacheDirectoryPath(ContextProxy.instance.globalStorageUri.fsPath)
...
export const getModels = async (options: GetModelsOptions): Promise<ModelRecord> => {
	...
	try {
		switch (provider) {
			case "openrouter":
				// oacode_change start: base url and bearer token
		...
		/* oacode_change: skip useless file IO
