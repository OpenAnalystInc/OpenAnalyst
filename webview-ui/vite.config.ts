...
export default defineConfig(({ mode }) => {
	...
	// oacode_change start - read package.json fresh every time to avoid caching issues
	const getPkg = () => {
		try {
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("Oa-Code-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				// Route to browser-safe version for webview
				"@roo/modes": resolve(__dirname, "../src/shared/modes.browser.ts"),
		...
		optimizeDeps: {
			...
			exclude: ["@vscode/codicons", "vscode-oniguruma", "shiki", "vscode" /*oacode_change*/],
		},
