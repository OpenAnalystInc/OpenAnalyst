...
// import RooHero from "@src/components/welcome/RooHero" // oacode_change: unused
// import RooTips from "@src/components/welcome/RooTips" // oacode_change: unused
// import RooCloudCTA from "@src/components/welcome/RooCloudCTA" // oacode_change: unused
import { StandardTooltip } from "@src/components/ui"
import { useAutoApprovalState } from "@src/hooks/useAutoApprovalState"
import { useAutoApprovalToggles } from "@src/hooks/useAutoApprovalToggles"

import TelemetryBanner from "../common/TelemetryBanner" // oacode_change: deactivated for now
// import VersionIndicator from "../common/VersionIndicator" // oacode_change: unused
...
import BottomControls from "../oacode/BottomControls" // oacode_change
import SystemPromptWarning from "./SystemPromptWarning"
import { showSystemNotification } from "@/oacode/helpers" // oacode_change
// import ProfileViolationWarning from "./ProfileViolationWarning" oacode_change: unused
import { CheckpointWarning } from "./CheckpointWarning"
import { IdeaSuggestionsBox } from "../oacode/chat/IdeaSuggestionsBox" // oacode_change
import { OacodeNotifications } from "../oacode/OacodeNotifications" // oacode_change
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
					...
					switch (lastMessage.ask) {
						...
						case "tool":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("oacode:notifications.toolRequest")) // oacode_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("tool")
						...
						case "browser_action_launch":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("oacode:notifications.browserAction")) // oacode_change
							}
							setSendingDisabled(isPartial)
						...
						case "command":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("oacode:notifications.command")) // oacode_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("command")
							setEnableButtons(!isPartial)
						...
						case "condense":
							...
							setEnableButtons(!isPartial)
							setPrimaryButtonText(t("oacode:chat.condense.condenseConversation"))
							setSecondaryButtonText(undefined)
							break
						// oacode_change end
					}
	...
	useEffect(() => {
		...
		return () => {
			isProcessingQueueRef.current = false
			// oacode_change start // Clear any pending queue processing
	...
	const handlePrimaryButtonClick = useCallback(
		...
		[clineAsk, startNewTask, lastMessage?.text], // oacode_change: add lastMessage?.text
	)
	...
	const handleTaskCloseButtonClick = useCallback(() => startNewTask(), [startNewTask]) // oacode_change
	...
	const isAllowedCommand = useCallback(
		(message: ClineMessage | undefined): boolean => {
			// oacode_change start wrap in try/catch
	...
	const groupedMessages = useMemo(() => {
		...
		visibleMessages.forEach((message: ClineMessage) => {
			// oacode_change start: upstream pr https://github.com/RooCodeInc/Roo-Code/pull/5452
	...
	const itemContent = useCallback(
		(index: number, messageOrGroup: ClineMessage | ClineMessage[]) => {
			...
			return (
				<ChatRow
					...
					highlighted={highlightedMessageIndex === index} // oacode_change: add highlight prop
		...
		[
			...
			highlightedMessageIndex, // oacode_change: add highlightedMessageIndex
	...
	useEffect(() => {
		...
		window.addEventListener("wheel", handleWheel, { passive: true }) // oacode_change
		return () => {
			window.removeEventListener("keydown", handleKeyDown)
			window.removeEventListener("wheel", handleWheel) // oacode_change
		}
	}, [handleKeyDown, handleWheel]) // oacode_change
	...
	return (
		<div
			...
			{task ? (
				...
				<div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-4 relative">
					...
					<div className="w-full h-full flex flex-col gap-4 px-3.5 transition-all duration-300">
						...
						{/* oacode_change start: OacodeNotifications + Layout fixes */}
						{telemetrySetting !== "unset" && <OacodeNotifications />}
						<div className="flex flex-grow flex-col justify-center gap-4">
							...
							{taskHistory.length === 0 && <IdeaSuggestionsBox />} {/* oacode_change */}
							{/*<div className="mb-2.5">
								{cloudIsAuthenticated || taskHistory.length < 4 ? <RooTips /> : <RooCloudCTA />}
							</div> oacode_change: do not show */}
			...
			*/}
			{/* oacode_change: added settings toggle for this */}
				...
				<>
					<div className="grow flex" ref={scrollContainerRef}>
						<Virtuoso
							...
							increaseViewportBy={{ top: 400, bottom: 400 }} // oacode_change: use more modest numbers to see if they reduce gray screen incidence
			...
			/>
			{/* oacode_change: added settings toggle the profile and model selection */}
			<BottomControls showApiConfig />
			{/* oacode_change: end */}

			{/* oacode_change: disable {isProfileDisabled && (
				<div className="px-3">
