...
import { DropdownOptionType, Button, StandardTooltip } from "@/components/ui" // oacode_change
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		// oacode_change begin: remove button from chat when it gets to small
		...
		const handleInputChange = useCallback(
			(e: React.ChangeEvent<HTMLTextAreaElement>) => {
				...
				if (showMenu) {
					// oacode_change start - check lastAtIndex before handling slash commands
					const lastAtIndex = newValue.lastIndexOf("@", newCursorPosition - 1)

					// if (newValue.startsWith("/")) { ⚠️ oacode_change added lastAtIndex check
		...
		const handleBlur = useCallback(() => {
			...
			if (!isMouseDownOnMenu) {
				setShowContextMenu(false)
				setShowSlashCommandsMenu(false) // oacode_change: pull slash commands from Cline
			}
		...
		const renderNonEditModeControls = () => (
			// oacode_change move thumbnails to bottom
			...
			<div
				...
				<div className={cn("flex", "items-center", "gap-1", "min-w-0")}>
					<div className="shrink-0">
						{/* oacode_change start: OaModeSelector instead of ModeSelector */}
						<OaModeSelector
							value={mode}
							onChange={setMode}
							modeShortcutText={modeShortcutText}
				...
				{/* oacode_change: hidden on small containerWidth
		...
		const renderTextAreaSection = () => (
			<div
				...
				<DynamicTextArea
					...
					// oacode_change: combine placeholderText and placeholderBottomText here
				...
				/>
				{/* oacode_change {Transparent overlay at bottom of textArea to avoid text overlap } */}
				<div
				...
				{/* oacode_change: position tweaked */}
				...
				{/* oacode_change: position tweaked, rtl support */}
				...
				{!inputValue && (
					<div
						...
						{/* oacode_change {placeholderBottomText} */}
					</div>
				)}
			</div>
		)
			...
			<div
				...
				<div className="relative">
					<div
						...
						{/* oacode_change: renderNonEditModeControls moved */}
