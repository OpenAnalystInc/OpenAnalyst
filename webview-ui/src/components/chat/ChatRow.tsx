...
// import { Mode } from "@roo/modes" // oacode_change
...
// import { Button } from "@src/components/ui" // oacode_change

// import ChatTextArea from "./ChatTextArea" // oacode_change
...
// import Thumbnails from "../common/Thumbnails" // oacode_change
import McpResourceRow from "../mcp/McpResourceRow"

// import { Mention } from "./Mention" // oacode_change
import { CheckpointSaved } from "./checkpoints/CheckpointSaved"
import { FollowUpSuggest } from "./FollowUpSuggest"
import { LowCreditWarning } from "../oacode/chat/LowCreditWarning" // oacode_change
...
import { NewTaskPreview } from "../oacode/chat/NewTaskPreview" // oacode_change
import { OaChatRowGutterBar } from "../oacode/chat/OaChatRowGutterBar" // oacode_change
...
interface ChatRowProps {
	...
	highlighted?: boolean // oacode_change: Add highlighted prop
	onChatReset?: () => void // oacode_change
...
const ChatRow = memo(
	(props: ChatRowProps) => {
		const { highlighted } = props // oacode_change: Add highlighted prop
		const { showTaskTimeline } = useExtensionState() // oacode_change: Used by OaChatRowGutterBar
		...
		const [chatrow, { height }] = useSize(
			<div
				// oacode_change: add highlighted className
				className={cn(
					`px-[15px] py-[10px] pr-[6px] relative ${highlighted ? "animate-message-highlight" : ""}`,
				)}>
				{showTaskTimeline && <OaChatRowGutterBar message={message} />}
...
export const ChatRowContent = ({
	...
	const [isEditing, _setIsEditing] = useState(false) // oacode_change
	// const [editedContent, setEditedContent] = useState("") // oacode_change
	// const [editMode, setEditMode] = useState<Mode>(mode || "code") // oacode_change
	const [_editImages, setEditImages] = useState<string[]>([]) // oacode_change
		...
		case "say":
			switch (message.say) {
							...
							<div
								...
								<div style={{ display: "flex", alignItems: "center", gap: "10px", flexGrow: 1 }}>
									...
									{title}
									{
										// oacode_change start
										!cost && usageMissing && (
											<StandardTooltip content={t("oacode:pricing.costUnknownDescription")}>
												<VSCodeBadge className="whitespace-nowrap">
													<span className="codicon codicon-warning pr-1"></span>
													{t("oacode:pricing.costUnknown")}
												</VSCodeBadge>
											</StandardTooltip>
										)
										// oacode_change end
				...
				case "user_feedback":
					// oacode_change start
					return (
						<OaChatRowUserFeedback message={message} isStreaming={isStreaming} onChatReset={onChatReset} />
					)
				// oacode_change end
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									{t("oacode:chat.condense.wantsToCondense")}
								</span>
							</div>
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									OaCode wants to create a Github issue:
								</span>
							</div>
