 ...
 */
export const CreateRulePopover: React.FC<CreateRulePopoverProps> = ({ open, onOpenChange, className, trigger }) => {
				...
				<div ref={componentRef}>
					...
					<div className="px-4 py-3 border-b border-vscode-dropdown-border">
						<div className="flex items-center justify-between">
							<h3 className="font-medium text-sm text-vscode-foreground">Create New Rule</h3>
							<button
								onClick={handleClose}
					...
					<div className="p-4 space-y-4">
						...
						<div>
							<label className="text-sm font-medium text-vscode-foreground mb-2 block">Rule Scope</label>
							...
							<p className="text-xs text-vscode-descriptionForeground mt-1">
								{isGlobal ? "Apply to all projects and workspaces" : "Apply only to current workspace"}
							</p>
						</div>
						...
						<form onSubmit={handleSubmit} className="space-y-3">
							<div>
								...
								{error && <p className="text-xs text-vscode-errorForeground mt-1">{error}</p>}
							</div>

							{/* Submit Button */}
							<div className="flex justify-end pt-2">
								<Button type="submit" size="sm" disabled={!filename.trim() || isSubmitting}>
