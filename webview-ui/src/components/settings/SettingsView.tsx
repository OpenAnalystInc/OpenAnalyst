...
import { DisplaySettings } from "./DisplaySettings" // oacode_change
...
import McpView from "../oacodeMcp/McpView" // oacode_change
import deepEqual from "fast-deep-equal" // oacode_change
import { GhostServiceSettingsView } from "../oacode/settings/GhostServiceSettings" // oacode_change
...
type SectionName = (typeof sectionNames)[number] // oacode_change
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	const { uiKind } = extensionState // oacode_change
	...
	const { oacodeToken, openRouterApiKey, glamaApiKey, requestyApiKey } = extensionState.apiConfiguration ?? {}
	useEffect(() => {
	...
	}, [oacodeToken, openRouterApiKey, glamaApiKey, requestyApiKey])
	...
	const handleSubmit = () => {
		if (isSettingValid) {
			...
			vscode.postMessage({ type: "terminalCommandApiConfigId", text: terminalCommandApiConfigId || "" }) // oacode_change
			...
			vscode.postMessage({ type: "showAutoApproveMenu", bool: showAutoApproveMenu }) // oacode_change
			...
			vscode.postMessage({ type: "allowVeryLargeReads", bool: allowVeryLargeReads }) // oacode_change
			...
			vscode.postMessage({ type: "showTaskTimeline", bool: showTaskTimeline }) // oacode_change
			...
			vscode.postMessage({ type: "systemNotificationsEnabled", bool: systemNotificationsEnabled }) // oacode_change
			vscode.postMessage({ type: "ghostServiceSettings", values: ghostServiceSettings }) // oacode_change
	...
	const sections: { id: SectionName; icon: LucideIcon }[] = useMemo(
		() => [
			...
			{ id: "display", icon: Monitor }, // oacode_change
			...(extensionState.experiments?.inlineAssist ? [{ id: "ghost" as const, icon: Bot }] : []), // oacode_change
			...
			<div ref={containerRef} className={cn(settingsTabsContainer, isCompactMode && "narrow")}>
							...
							<TabTrigger
								...
								<div className={cn("flex items-center gap-2", isCompactMode && "justify-center")}>
									...
									<span className="tab-label">
										{id === "mcp"
											? t(`oacode:settings.sections.mcp`)
											: id === "ghost"
												? t(`oacode:ghost.title`)
				...
				<TabContent className="p-0 flex-1 overflow-auto">
					...
					{activeTab === "autoApprove" && (
						<AutoApproveSettings
							showAutoApproveMenu={showAutoApproveMenu} // oacode_change
					...
					{/* oacode_change start display section */}
					{activeTab === "display" && (
						<DisplaySettings
					...
					{/* oacode_change end display section */}
					...
					{activeTab === "contextManagement" && (
						<ContextManagementSettings
							...
							allowVeryLargeReads={allowVeryLargeReads /* oacode_change */}
					...
					{activeTab === "terminal" && (
						<TerminalSettings
							...
							terminalCommandApiConfigId={terminalCommandApiConfigId} // oacode_change
							setCachedStateField={setCachedStateField}
						/>
					)}
					...
					{activeTab === "experimental" && (
						<ExperimentalSettings
							...
							apiConfiguration={apiConfiguration /*oacode_change*/}
							setApiConfigurationField={setApiConfigurationField /*oacode_change*/}
						/>
					)}
