...
import { IconProps } from "@radix-ui/react-icons/dist/types" // oacode_change
...
export const SelectDropdown = React.memo(
	React.forwardRef<React.ElementRef<typeof PopoverTrigger>, SelectDropdownProps>(
		(
			...
			const [open, setOpen] = React.useState(initiallyOpen) // oacode_change
				...
				<Popover open={open} onOpenChange={onOpenChange} data-testid="dropdown-root">
					...
					<PopoverContent
						...
						<div className="flex flex-col w-full">
							...
							{/* oacode_change: different max height: max-h-60 */}
										...
										{groupedOptions.map((option, index) => {
											...
											return (
												<div
													...
													className={cn(
														"text-sm cursor-pointer flex items-center", // oacode_change
														option.disabled
															? "opacity-50 cursor-not-allowed"
