...
// import { MarketplaceViewStateManager } from "./components/marketplace/MarketplaceViewStateManager" // oacode_change: rendered in settings
...
import WelcomeView from "./components/oacode/Welcome/WelcomeView" // oacode_change
import ProfileView from "./components/oacode/profile/ProfileView" // oacode_change
import McpView from "./components/mcp/McpView"
// import { MarketplaceView } from "./components/marketplace/MarketplaceView" // oacode_change: rendered in settings
import ModesView from "./components/modes/ModesView"
import { HumanRelayDialog } from "./components/human-relay/HumanRelayDialog"
import BottomControls from "./components/oacode/BottomControls" // oacode_change
import { MemoryService } from "./services/MemoryService" // oacode_change
import { DeleteMessageDialog, EditMessageDialog } from "./components/chat/MessageModificationConfirmationDialog"
import ErrorBoundary from "./components/ErrorBoundary"
// import { AccountView } from "./components/account/AccountView" // oacode_change: we have our own profile view
...
const App = () => {
	const {
		...
		telemetryKey,
		machineId,
		// cloudUserInfo, // oacode_change not used
		// cloudIsAuthenticated, // oacode_change not used
		renderContext,
		mdmCompliant,
		apiConfiguration, // oacode_change
	} = useExtensionState()

	// Create a persistent state manager
	// const marketplaceStateManager = useMemo(() => new MarketplaceViewStateManager(), []) // oacode_change: rendered in settings
	...
	const chatViewRef = useRef<ChatViewRef & { focusInput: () => void }>(null) // oacode_change
	...
	// oacode_change start
	const telemetryDistinctId = useOaIdentity(apiConfiguration?.oacodeToken ?? "", machineId ?? "")
	useEffect(() => {
		if (didHydrateState) {
	...
	return showWelcome ? (
		...
		<>
			...
			{tab === "settings" && (
				<SettingsView ref={settingsRef} onDone={() => switchTab("chat")} targetSection={currentSection} /> // oacode_change
			)}
			{/* oacode_change: add profileview */}
			{tab === "profile" && <ProfileView onDone={() => switchTab("chat")} />}
			{/* oacode_change: rendered in settings */}
			{/* {tab === "marketplace" && (
				<MarketplaceView stateManager={marketplaceStateManager} onDone={() => switchTab("chat")} />
			)} */}
			{/* oacode_change: we have our own profile view */}
			{/* {tab === "account" && (
