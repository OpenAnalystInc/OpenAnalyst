...
export const createPromptActivationService = (): PromptActivationService => {
	...
	const activatePromptBlock = (
		...
		if (existingInCategory) {
			...
			console.log(`[PromptActivationService] CATEGORY CONFLICT: Replacing ${existingInCategory.block.name} with ${blockName} in category ${block.category}`)
			...
			setActiveBlocks(prev => prev.map(ab => 
				ab.block.category === block.category ? newActiveBlock : ab
			))
		} else {
