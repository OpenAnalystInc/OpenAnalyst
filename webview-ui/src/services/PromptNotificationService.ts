...
export class PromptNotificationService {
	...
	async showActivationNotification(
		...
		if (result.success) {
			...
			await this.showNotification(notificationType, result.userMessage || `✅ Activated ${blockName}`, {
				...opts,
				context: {
					blockName,
					source,
	...
	async showCategoryConflictInfo(
		...
		await this.showNotification("conflict-info", message, {
	...
	async showDeactivationNotification(
		...
		const notificationType: PromptNotificationType = success ? "deactivation-success" : "activation-error"

		await this.showNotification(notificationType, message, {
			showSystemNotification: true,
			showConsoleLog: true,
			...options,
			context: {
	...
	async showValidationError(
		...
		await this.showNotification("validation-error", message, {
			showSystemNotification: true,
			showConsoleLog: true,
			...options,
			context: {
