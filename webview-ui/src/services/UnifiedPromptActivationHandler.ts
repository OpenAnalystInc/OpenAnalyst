...
export class UnifiedPromptActivationHandler {
	...
	async activatePromptBlock(
		...
		if (!consistencyCheck.isConsistent) {
			console.error(
				`[UnifiedActivationHandler] PHASE 5.5: System prompt consistency validation failed:`,
		...
		try {
			...
			if (conflictInfo) {
				console.log(
					`[UnifiedActivationHandler] CATEGORY REPLACEMENT: Will replace '${conflictInfo.existingBlock.block.name}' with '${blockName}' in category '${conflictInfo.category}'`,
				)
			...
			if (conflictInfo) {
				...
				await this.performCategoryReplacement(blockName, targetBlock, conflictInfo, context, source, options)
			} else {
				// PHASE 5.2: Standard activation (no replacement needed)
				// PHASE 5.5 CRITICAL: Both methods use the same backend activation path
				console.log(
					`[UnifiedActivationHandler] PHASE 5.5: Standard activation - ensuring identical system prompt update for ${source}`,
				)
				...
				console.log(
					`[UnifiedActivationHandler] PHASE 5.5: System prompt updated identically via PromptActivationService`,
				)
			}
			...
			const userMessage =
				options.successMessage || this.generateReplacementMessage(blockName, targetBlock, conflictInfo, true)
			...
			if (options.showFeedback !== false) {
				await promptNotificationService.showActivationNotification(result, blockName, source, {
	 ...
	 */
	canActivateBlock(blockName: string, context: ActivationContext): { canActivate: boolean; reason?: string } {
		// Block must exist
	...
	private async performCategoryReplacement(
		...
		console.log(
			`[UnifiedActivationHandler] Category '${conflictInfo.category}': ${conflictInfo.existingBlock.block.name} → ${blockName}`,
		)
	...
	getSystemPromptUpdateInfo(
		...
		const affectedCategories = conflictInfo ? [conflictInfo.category] : [targetBlock.category]
