...
// oacode_change: this file was pulled from Cline and adjusted for us
...
export function getSupportedSlashCommands(
	...
	const baseCommands: SlashCommand[] = [
		...
		{
			name: "newrule",
			description: "Create a new Oa rule with context from your conversation",
		},
		{ name: "reportbug", description: "Create a OaCode GitHub issue" },
	...
	const modeCommands = getAllModes(customModes).map((mode) => ({
		name: mode.slug,
		description: `Switch to ${mode.name.replace(/^[💻🏗️❓🪲🪃]+ /, "")} agent`,
	}))
