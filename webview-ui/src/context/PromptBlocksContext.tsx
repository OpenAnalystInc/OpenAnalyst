...
export const PromptBlocksProvider: React.FC<PromptBlocksProviderProps> = ({ children }) => {
		...
		const handleMessage = (event: MessageEvent) => {
			...
			switch (message.type) {
				...
				case "activePromptBlocksUpdated":
					if (__DEV__) {
						console.log("[PromptBlocksContext] Active blocks updated:", message.activeBlocks?.length || 0, "active")
					}
	 ...
	 */
	const activateBlocksInCategory = async (category: string, blockName: string, variables?: Record<string, string>) => {
	 ...
	 */
	const replaceActiveBlock = async (currentBlockName: string, newBlockName: string, variables?: Record<string, string>) => {
